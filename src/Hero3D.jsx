...
export default function Hero3D() {
  ...
  useEffect(() => {
    ...
    // Lighting
    const ambient = new THREE.AmbientLight(0x1a2a33, 1.4);
    scene.add(ambient);
    const key = new THREE.PointLight(0xb2d5e5, 3.2, 30);
    key.position.set(4, 4, 6);
    scene.add(key);
    const rim = new THREE.PointLight(0xdcebf2, 1.6, 30);
    ...
    const wireMat = new THREE.MeshBasicMaterial({ color: 0xb2d5e5, wireframe: true, transparent: true, opacity: 0.35 });
    const wire = new THREE.Mesh(wireGeo, wireMat);
    group.add(wire);

    // Orbiting rings (torus)
    const ringMat = new THREE.MeshStandardMaterial({ color: 0xb2d5e5, metalness: 0.9, roughness: 0.3, emissive: 0x14232b, emissiveIntensity: 0.3 });
    ...
    const particleMat = new THREE.MeshStandardMaterial({ color: 0xdcebf2, emissive: 0xcfe6f0, emissiveIntensity: 1.2, metalness: 0.6, roughness: 0.2 });
    for (let i = 0; i < 8; i++) {
